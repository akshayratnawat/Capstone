    ...
    while robot.step(time_step) != -1:
        ...
        if len(camera.getRecognitionObjects()) > 0:
            ...
            if ( x/y >= 0.5) :
                ...
                left_speed = max_speed
                right_speed = max_speed
                if min(sensor_values_print) <= 1000:
                    left_speed = 0
                    right_speed = 0
...
def move_around_object(robot):
    ...
    while robot.step(time_step) != -1:
        ...
        left_obstacle = (prox_sensors[5].getValue() < 1500)
        left_corner_obstacle = prox_sensors[6].getValue() < 1500

        right_obstacle = prox_sensors[2].getValue() < 1500
        #right_corner_obstacle = prox_sensors[1].getValue() > 1800

        front_obstacle = (prox_sensors[7].getValue() <= 1500)
